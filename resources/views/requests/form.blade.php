...
<form class="registry-card form-panel stepped-form" method="POST" enctype="multipart/form-data" action="{{ $editing?route('requests.update',$requestItem):route('requests.store') }}" novalidate>
 ...
 <section class="form-step-panel" data-step-panel="3"><div class="form-section"><h2>Ўлчам ва конструкция</h2><div class="form-grid two">
  <label class="wide">Реклама конструкцияси тури<select name="advertising_type" required><option value="">Танланг</option>@foreach($advertisingTypes as $k=>$v)<option value="{{ $k }}" @selected($field('advertising_type')===$k)>{{ $v }}</option>@endforeach</select>@error('advertising_type')<span>{{ $message }}</span>@enderror</label>
  ...
  <label>Паспорт маълумотлари<textarea name="passport_details">{{ $field('passport_details') }}</textarea>@error('passport_details')<span>{{ $message }}</span>@enderror</label>
  ...
  <label>Шартнома суммаси<input name="contract_amount" type="number" step="0.01" min="0" value="{{ $field('contract_amount') }}">@error('contract_amount')<span>{{ $message }}</span>@enderror</label>
 </div></div></section>
 <section class="form-step-panel" data-step-panel="4"><div class="form-section"><h2>Харита</h2><p class="map-help">Харитадан битта локацияни белгиланг. Маркерни суриб аниқлаштириш мумкин.</p><div id="location-map" class="leaflet-map" data-lat="{{ $field('latitude') }}" data-lng="{{ $field('longitude') }}"></div><input type="hidden" id="latitude" name="latitude" value="{{ $field('latitude') }}"><input type="hidden" id="longitude" name="longitude" value="{{ $field('longitude') }}">@error('latitude')<p class="field-error">{{ $message }}</p>@enderror @error('longitude')<p class="field-error">{{ $message }}</p>@enderror</div></section>
