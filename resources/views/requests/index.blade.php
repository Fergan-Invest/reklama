...
@php
    $fileTypeLabels = [
        'warning_letter_file' => 'Огоҳлантириш',
        'lease_document_file' => 'Ижара',
    ];
@endphp
...
<section class="page-title compact-title">
    <div>
        <h1>Реклама объектлари</h1>
    </div>
    @can('create', App\Models\RegistryRequest::class)
        <a class="primary-button" href="{{ route('requests.create') }}">Объект қўшиш</a>
    @endcan
</section>

<form class="panel filters soft-panel" method="GET">
    <input name="q" value="{{ request('q') }}" placeholder="СТИР, телефон ёки эгаси бўйича қидириш">
...
@if($requests->isEmpty())
    <section class="empty-state-card" aria-label="Ma'lumot yo'q">
        ...
        <p>Ҳали бирорта реклама объекти киритилмаган.</p>
        @can('create', App\Models\RegistryRequest::class)
            <a class="primary-button empty-action" href="{{ route('requests.create') }}">Объект қўшиш</a>
        @endcan
    </section>
@else
    <section class="panel table-panel registry-card">
        <div class="table-wrap">
            <table class="registry-table">
                <thead><tr><th>Т/р</th><th>Эгаси</th><th>Манзил</th><th>Конструкция</th><th>Ўлчам</th><th>Файллар</th><th>Сана</th><th></th></tr></thead>
                <tbody>
                @foreach($requests as $item)
                    ...
                    <tr class="clickable-row" data-href="{{ route('requests.show', $item) }}" onclick="window.location=this.dataset.href">
                        ...
                        <td>{{ \App\Models\RegistryRequest::ADVERTISING_TYPES[$item->advertising_type] ?? '-' }}</td>
                        <td>{{ $item->area_length }} × {{ $item->area_width }} м</td>
                        <td>
